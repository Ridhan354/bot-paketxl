...
Skrip `bot_xlremainder.sh` akan memasang bot ke dalam `/opt/bot-xlreminder/`, membuat virtual environment, serta menyiapkan service `systemd` agar bot mudah dijalankan.
...
1. **Install / Update bot** – menyalin berkas ke `/opt/bot-xlreminder/`, memasang dependensi, dan membuat service `systemd`.
...
5. **Edit BOT API token / ID admin** – memperbarui nilai `BOT_TOKEN`, `API_TEMPLATE`, atau `ADMIN_IDS` di berkas `.env` tanpa harus mengedit manual.
...
python /opt/bot-xlreminder/xl_bot.py
```
